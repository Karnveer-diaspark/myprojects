...
const openconfirm = {
  ...
  beforeEach(async(() => {
    TestBed.configureTestingModule({
      ...
      providers: [{ provide: MatTableDataSource, useValue: dialogMock }, { provide: MatDialogRef, entryComponents: ConfirmDialogComponent, useValue: dialogMock }, DialogService, { provide: DialogService, useValue: openforms, entryComponents: { CustomerDetailsComponent, ConfirmDialogComponent } }, { provide: MatDialog, useValue: {openforms,openconfirm} }, NotificationsService, CustomerService, MatSnackBar, MatTableDataSource],
  ...
  it('should execute onCreate()', () => {
    let cId = 0;
    spyOn(openforms, 'openForm').and.callThrough();
   let afterfunc = openforms.openForm();
    const response: Customer[] = [];
    spyOn(afterfunc, 'afterClosed').and.returnValue(of(response));
    component.onCreate();
  });
  ...
  it('should execute delete()', () => {
    ...
    spyOn(openconfirm, 'openConfirmDialog').and.callThrough()
    expect(component).toBeTruthy();
  });
  ...
  it('should execute onEdit()', () => {
    ...
    spyOn(openforms, 'openForm').and.callThrough();
    let forsubscribe = component.dialogservice.openForm(cId);
    spyOn(serviceDialog, 'openForm');
    spyOn(forsubscribe, 'afterClosed').and.returnValue(of())
    component.getcustomer();
    expect(component).toBeTruthy();
  });
});
